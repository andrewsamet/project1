...
# plt.plot(df['x'], df['y_pred'], color='red', label=f'Fit: y = {slope:.2f}x + {intercept:.2f}')
...
#     'height': [5, 6, 4, 5, 6, 7],
#     'age': [25, 30, 35, 23, 31, 56],
#     'city': [5, 'Paris', 'Berlin', 'London', 'Paris', 'London'],
# }
#
# df = pd.DataFrame(data)
# print(df)
...
# intercept = model.intercept_
#
# plt.scatter(df[['X']], df['age'], label = 'data')
# plt.plot(df['height'], df['y_pred'], color = 'yellow', label = f'Fit: y = {slope:.2f}x {intercept:.2f}')
# plt.legend()
# plt.xlabel('Height')
# plt.ylabel('Age')
# plt.grid(True)
